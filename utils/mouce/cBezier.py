...
class bezierTrajectory:
    ...
    def trackArray(self, start, end, numberList=50, le=1, deviation=0, bias=0.5, type=0, cbb=0, yhh=10):
        '''
    ...
    # return the offset for selenium
    def return_offset_cbezier(self, start, end, numberList=50, le=1, deviation=4, bias=0.5, type=2, cbb=2, yhh=10):
        '''
        ...
        '''
        # 修改为相对距离
        end = [end[i]-start[i] for i in range(len(start))]
        start = [0, 0] # 修订初值
        result_dict = self.trackArray(start, end, numberList, le, deviation, bias, type, cbb, yhh)
        track_array = result_dict['trackArray']

        def list_element_minis(before_list: list, then_list: list) -> list:
            result_list = []
            for element_id in range(len(before_list)):
                result_list.append(int(then_list[element_id]) - int(before_list[element_id]))
            return result_list

        offset_array = []
        for track_id in range(1, len(track_array)):
            offset_array.append(list_element_minis(track_array[track_id - 1],track_array[track_id]))

        # 修正元素
        sum_x, sum_y = 0, 0
        for x, y in offset_array:
            sum_x += x
            sum_y += y
        offset_array.append([end[0] - sum_x, end[1] - sum_y])  # 在列表后面加入补偿元素
        while offset_array[-1] == [0, 0]:
            offset_array.pop()
        return offset_array
